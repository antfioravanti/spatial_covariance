...
c = 1
sigma = 1
#l = 4 # should increase with increasing grid size
final_df = data.frame()
...
for(lambda in lambdas){
  ...
  frob_norm_sepkron = numeric(length(Ns))
  selected_l1 = numeric(length(Ns))
  selected_l2 = numeric(length(Ns))
  ...
  for (iter in 1:length(Ns)){
    ...
    # Select empirical bandwidth
    L = bandwidth_selection_spatial(GammaEst,
                                   n1 = grid_size,
                                   n2 = grid_size,
                                   C0 = 2,
                                   c_ef = 1)
    ...
    kappa_ij_vector = sapply(seq_len(nrow(M_ij_matrix)), function(idx) {
      # extract the d-dimensional lag
      x_vec = M_ij_matrix[idx, ]  
      # compute the product of tapers across dimensions
      flat_top_taper_multi(x_vec,
                           c    = c,
    ...
    SepResults= Tapered_Sep_Autocovariance_Kron_v2(X,
                                                   L=unlist(L, use.names = F),
                                                   c=c,
                                                   type=type)
    ...
    frob_norm_sepkron[iter] = norm(true_cov - GammaEstTaperSep, type = "F")
    selected_l1[iter] = unlist(L, use.names = F)[1]
    selected_l2[iter] = unlist(L, use.names = F)[2]
    toc()
  }
  toc()
...
write.csv(final_df, paste0(wd, "/results/simulations_20250117.csv"))
